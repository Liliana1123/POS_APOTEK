...
@section('content')
<!-- Page Header -->
<x-page-header title="Edit Custom Discount" subtitle="Ubah skema promo diskon, masa aktif, atau filter cakupan barang.">
    <a href="{{ route('custom-discount.index') }}" class="btn-secondary flex items-center gap-1.5">
        <x-heroicon-o-arrow-left class="w-4 h-4" />
        <span>Kembali</span>
    </a>
</x-page-header>

<form action="{{ route('custom-discount.update', $customDiscount) }}" method="POST" class="card-base max-w-2xl space-y-5">
    @csrf
    @method('PUT')
    ...
    <div>
        ...
        <input type="text" name="nama" value="{{ old('nama', $customDiscount->nama) }}" required placeholder="Contoh: Promo Spesial Ulang Tahun Apotek" class="form-input @error('nama') error @enderror">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
            ...
            <input type="number" name="persentase" value="{{ old('persentase', $customDiscount->persentase) }}" min="0" max="50" required class="form-input font-mono @error('persentase') error @enderror">
            @error('persentase') <p class="text-red-600 text-xs mt-1">{{ $message }}</p> @enderror
        </div>
        <div>
            ...
            @error('aktif') <p class="text-red-600 text-xs mt-1">{{ $message }}</p> @enderror
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
            ...
            <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', $customDiscount->tanggal_mulai->format('Y-m-d')) }}" required class="form-input @error('tanggal_mulai') error @enderror">
        ...
        <div>
            ...
            <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai', $customDiscount->tanggal_selesai->format('Y-m-d')) }}" required class="form-input @error('tanggal_selesai') error @enderror">
    ...
    <div>
        ...
        @error('cakupan') <p class="text-red-600 text-xs mt-1">{{ $message }}</p> @enderror
    </div>

    <!-- Kategori Selection Wrapper -->
    <div id="section-kategori" class="hidden border-t border-gray-100 pt-3 space-y-1.5">
        <label class="block text-xs font-semibold text-gray-700 mb-1.5 font-sans">Pilih Kategori</label>
        <div class="max-h-40 overflow-y-auto border border-gray-200 rounded-lg p-3 grid grid-cols-1 sm:grid-cols-2 gap-2 bg-gray-50">
    ...
    <div id="section-barang" class="hidden border-t border-gray-100 pt-3 space-y-2">
        ...
        <div class="max-h-48 overflow-y-auto border border-gray-200 rounded-lg p-3 space-y-1.5 bg-gray-50" id="barang-list-container">
            @foreach ($barangs as $b)
                <label class="barang-item flex items-center gap-2 text-xs cursor-pointer hover:bg-white p-1 rounded transition-colors" data-nama="{{ strtolower($b->nama) }}">
    ...
    <div class="flex items-center justify-end gap-3 border-t border-gray-100 pt-4">
        <a href="{{ route('custom-discount.index') }}" class="btn-secondary">Batal</a>
